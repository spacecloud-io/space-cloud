...
func removeRegistry(projectID string) error {
	...
	if err := operations.ApplySpec(token, account, specObj[0]); err != nil {
		return utils.LogError(fmt.Sprintf("Unable to remove project (%s) with docker registry urlby spec object with id %v type %v", projectID, specObj[0].Meta["id"], specObj[0].Type), err)
	}
