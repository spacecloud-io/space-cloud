...
func addRegistry(projectID string) error {
	...
	if len(specObj) == 0 {
		return utils.LogError("No project found", "add", "registry", fmt.Errorf("No project found with id (%s)", projectID))
	}
	if err != nil {
	...
	if err := cmd.ApplySpec(login.Token, account, specObj[0]); err != nil {
