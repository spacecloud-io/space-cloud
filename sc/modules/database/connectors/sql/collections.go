...
func (s *SQL) GetCollections(ctx context.Context) ([]model.DatabaseCollections, error) {
	...
	result := make([]model.DatabaseCollections, 0)
	for rows.Next() {
		...
		result = append(result, model.DatabaseCollections{TableName: tableName})
	}
