...
func (c *creationModule) addNotNull() string {
	...
	switch model.DBType(c.dbType) {
	case model.MySQL:
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " MODIFY " + c.ColumnName + " " + c.columnType + " NOT NULL"
	case model.Postgres:
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " ALTER COLUMN " + c.ColumnName + " SET NOT NULL"
	case model.SQLServer:
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " ALTER COLUMN " + c.ColumnName + " " + c.columnType + " NOT NULL"
	}
	return ""
}
...
func (c *creationModule) removeNotNull() string {
	switch model.DBType(c.dbType) {
	case model.MySQL:
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " MODIFY " + c.ColumnName + " " + c.columnType + " NULL"
	case model.Postgres:
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " ALTER COLUMN " + c.ColumnName + " DROP NOT NULL"
	case model.SQLServer:
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " ALTER COLUMN " + c.ColumnName + " " + c.columnType + " NULL" // adding NULL solves a bug that DateTime type is always not nullable even if (!) is not provided
	}
	return ""
}
...
func (c *creationModule) addNewColumn() string {
	switch model.DBType(c.dbType) {
	case model.MySQL:
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " ADD " + c.ColumnName + " " + c.columnType
	case model.Postgres:
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " ADD COLUMN " + c.ColumnName + " " + c.columnType
	case model.SQLServer:
		if c.columnType == "timestamp" && !c.realColumnInfo.IsFieldTypeRequired {
			return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " ADD " + c.ColumnName + " " + c.columnType + " NULL"
		}
		...
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " ADD " + c.ColumnName + " " + sqlDataType
	}
	return ""
}
...
func (c *creationModule) removeColumn(dbType string) []string {
	...
	return append(queries, "ALTER TABLE "+getTableName(dbType, c.dbName, c.TableName)+" DROP COLUMN "+c.ColumnName+"")
}
...
// 	case utils.MySQL:
// 		return "ALTER TABLE " + getTableName(dbAlias, c.dbName, c.TableName) + " ADD PRIMARY KEY (" + c.ColumnName + ")"
// 	case utils.Postgres:
// 		return "ALTER TABLE " + getTableName(dbAlias, c.dbName, c.TableName) + " ADD CONSTRAINT c_" + c.TableName + "_" + c.ColumnName + " PRIMARY KEY (" + c.ColumnName + ")"
// 	case utils.SQLServer:
// 		return "ALTER TABLE " + getTableName(dbAlias, c.dbName, c.TableName) + " ADD CONSTRAINT c_" + c.TableName + "_" + c.ColumnName + " PRIMARY KEY CLUSTERED (" + c.ColumnName + ")"
// 	}
// 	return ""
// }
...
// 	switch utils.DBAlias(dbAlias) {
// 	case utils.MySQL:
// 		return "ALTER TABLE " + getTableName(dbAlias, c.dbName, c.TableName) + " DROP PRIMARY KEY"
// 	case utils.Postgres:
// 		return "ALTER TABLE " + getTableName(dbAlias, c.dbName, c.TableName) + " DROP CONSTRAINT c_" + c.TableName + "_" + c.ColumnName
// 	case utils.SQLServer:
// 		return "ALTER TABLE " + getTableName(dbAlias, c.dbName, c.TableName) + " DROP CONSTRAINT c_" + c.TableName + "_" + c.ColumnName
// 	}
// 	return ""
...
func (c *creationModule) addForeignKey(dbType string) string {
	...
	if c.realColumnInfo.JointTable.OnDelete == "CASCADE" {
		return "ALTER TABLE " + getTableName(dbType, c.dbName, c.TableName) + " ADD CONSTRAINT " + c.realColumnInfo.JointTable.ConstraintName + " FOREIGN KEY (" + c.ColumnName + ") REFERENCES " + getTableName(dbType, c.dbName, c.realColumnInfo.JointTable.Table) + " (" + c.realColumnInfo.JointTable.To + ") " + "ON DELETE CASCADE"
	}
	return "ALTER TABLE " + getTableName(dbType, c.dbName, c.TableName) + " ADD CONSTRAINT " + c.realColumnInfo.JointTable.ConstraintName + " FOREIGN KEY (" + c.ColumnName + ") REFERENCES " + getTableName(dbType, c.dbName, c.realColumnInfo.JointTable.Table) + " (" + c.realColumnInfo.JointTable.To + ")"
}
...
func (c *creationModule) typeSwitch() string {
	...
	case bool:
		if model.DBType(c.dbType) == model.SQLServer {
			if v {
				return "1"
			}
			return "0"
...
func (c *creationModule) addDefaultKey() string {
	...
	switch model.DBType(c.dbType) {
	case model.MySQL:
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " MODIFY COLUMN " + c.ColumnName + " " + c.columnType + " DEFAULT(" + c.typeSwitch() + ")"
	case model.SQLServer:
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " ADD CONSTRAINT c_" + c.ColumnName + " DEFAULT " + c.typeSwitch() + " FOR " + c.ColumnName
	case model.Postgres:
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " ALTER COLUMN " + c.ColumnName + " SET DEFAULT " + c.typeSwitch()
	}
	return ""
}
...
func (c *creationModule) removeDefaultKey() string {
	switch model.DBType(c.dbType) {
	case model.MySQL:
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " ALTER " + c.ColumnName + " DROP DEFAULT"
	case model.Postgres:
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " ALTER COLUMN " + c.ColumnName + " DROP DEFAULT"
	case model.SQLServer:
		return "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " DROP CONSTRAINT c_" + c.ColumnName
	}
	return ""
}
...
func (c *creationModule) removeForeignKey() []string {
	...
	switch model.DBType(c.dbType) {
	case model.MySQL:
		return []string{"ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " DROP FOREIGN KEY " + c.currentColumnInfo.JointTable.ConstraintName, "ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " DROP INDEX " + c.currentColumnInfo.JointTable.ConstraintName}
	case model.Postgres:
		return []string{"ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " DROP CONSTRAINT " + c.currentColumnInfo.JointTable.ConstraintName}
	case model.SQLServer:
		return []string{"ALTER TABLE " + getTableName(c.dbType, c.dbName, c.TableName) + " DROP CONSTRAINT " + c.currentColumnInfo.JointTable.ConstraintName}
	}
	return nil
...
func addNewTable(ctx context.Context, logicalDBName, dbType, realColName string, realColValue model.FieldSchemas) (string, error) {
	...
	return `CREATE TABLE ` + getTableName(dbType, logicalDBName, realColName) + ` (` + primaryKeyQuery + strings.TrimSuffix(query, " ,") + `);`, nil
}
...
func getTableName(dbType, logicalDBName, table string) string {
	switch model.DBType(dbType) {
	...
	for _, indexInfo := range c.currentColumnInfo.IndexInfo {
		if indexInfo.IsIndex || indexInfo.IsUnique {
			if _, p := c.currentIndexMap[indexInfo.Group]; p {
				queries = append(queries, removeIndex(dbType, c.dbName, c.TableName, indexInfo.ConstraintName))
...
func addIndex(dbType, logicalDBName, tableName, indexName string, isIndexUnique bool, mapArray []*model.TableProperties) string {
	a := " ("
	...
	p := ""
	if isIndexUnique {
		p = "CREATE UNIQUE INDEX " + getIndexName(tableName, indexName) + " ON " + getTableName(dbType, logicalDBName, tableName) + a + ")"
	} else {
		p = "CREATE INDEX " + getIndexName(tableName, indexName) + " ON " + getTableName(dbType, logicalDBName, tableName) + a + ")"
	}
	return p
}

func removeIndex(dbType, logicalDBName, tableName, indexName string) string {

	switch model.DBType(dbType) {
	case model.MySQL:
		return "DROP INDEX " + indexName + " ON " + getTableName(dbType, logicalDBName, tableName)
	case model.SQLServer:
		return "DROP INDEX " + indexName + " ON " + getTableName(dbType, logicalDBName, tableName)
	case model.Postgres:
		indexname := indexName
		return "DROP INDEX " + getTableName(dbType, logicalDBName, indexname)
	}
	return ""
}
...
func getIndexMap(ctx context.Context, tableInfo model.FieldSchemas) (map[string]*indexStruct, error) {
...
// func getSchemaResponse(ctx context.Context, format, dbName, tableName string, ignoreForeignCheck bool, alreadyAddedTables map[string]bool, schemaResponse *[]interface{}) error {
