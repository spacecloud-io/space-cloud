...
func New(nodeID, clusterID, advertiseAddr, storeType, runnerAddr string, isDev bool, adminUserInfo *config.AdminUser, ssl *config.SSL) (*Server, error) {
	...
	helpers.Logger.LogInfo(helpers.GetInternalRequestID(), fmt.Sprintf("Creating a new server with id %s", nodeID), nil)
...
func (s *Server) Start(profiler bool, staticPath string, port int, restrictedHosts []string) error {
	...
	if s.ssl != nil && s.ssl.Enabled {
		...
		if s.ssl.Key != "none" && s.ssl.Crt != "none" {
			helpers.Logger.LogDebug(helpers.GetInternalRequestID(), "Adding existing certificates", nil)
			if err := s.modules.LetsEncrypt().AddExistingCertificate(s.ssl.Crt, s.ssl.Key); err != nil {
				return helpers.Logger.LogError(helpers.GetInternalRequestID(), "Could not log existing certificates", err, nil)
			}
		}

		go func() {
			// Start the server
			helpers.Logger.LogInfo(helpers.GetInternalRequestID(), "Starting https server on port: "+strconv.Itoa(port+4), nil)
