...
func initMissionContol(version string) (string, error) {
	...
	if os.IsNotExist(err) {
		helpers.Logger.LogInfo(helpers.GetInternalRequestID(), "Could not find mission control", nil)
		...
		helpers.Logger.LogInfo(helpers.GetInternalRequestID(), "Downloading...", nil)
		...
		helpers.Logger.LogInfo(helpers.GetInternalRequestID(), "Extracting...", nil)
		err = utils.Unzip(uiPath+".zip", uiPath)
		if err != nil {
			return "", err
		}
		helpers.Logger.LogInfo(helpers.GetInternalRequestID(), "Done...", nil)
