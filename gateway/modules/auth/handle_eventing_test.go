...
func TestModule_IsEventingOpAuthorised(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "got rule",
			m:    &Module{project: "some-project", eventingRules: map[string]*config.Rule{"some-type": &config.Rule{Rule: "authenticated"}}, secrets: []*config.Secret{{true, "mySecretkey"}}},
		...
		},
		{
			...
			m:       &Module{eventingRules: map[string]*config.Rule{"some-type": &config.Rule{}}, secrets: []*config.Secret{{true, "mySecretkey"}}},
		...
		},
		{
			...
			m:    &Module{project: "some-project", eventingRules: map[string]*config.Rule{"some-type": &config.Rule{Rule: "allow"}}, secrets: []*config.Secret{{true, "mySecretkey"}}},
			args: args{ctx: context.Background(), project: "some-project", token: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0b2tlbjEiOiJ0b2tlbjF2YWx1ZSIsInRva2VuMiI6InRva2VuMnZhbHVlIn0.h3jo37fYvnf55A63N-uCyLj9tueFwlGxEGCsf7gCjDc", event: &model.QueueEventRequest{Type: "some-type", Delay: 0, Payload: "something", Options: make(map[string]string)}},
		},
		{
		...
		},
		{
			...
			m:       &Module{project: "some-project", eventingRules: map[string]*config.Rule{"some-type": &config.Rule{Rule: "deny"}}, secrets: []*config.Secret{{true, "mySecretkey"}}},
			args:    args{ctx: context.Background(), project: "some-project", token: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0b2tlbjEiOiJ0b2tlbjF2YWx1ZSIsInRva2VuMiI6InRva2VuMnZhbHVlIn0.h3jo37fYvnf55A63N-uCyLj9tueFwlGxEGCsf7gCjDc", event: &model.QueueEventRequest{Type: "some-type", Delay: 0, Payload: "something", Options: make(map[string]string)}},
			wantErr: true,
		},
	}
