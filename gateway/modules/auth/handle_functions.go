...
func (m *Module) IsFuncCallAuthorised(ctx context.Context, project, service, function, token string, params interface{}) (*model.PostProcess, model.RequestParams, error) {
	...
	rule, err := m.getFunctionRule(ctx, project, service, function)
	if err != nil {
...
func (m *Module) getFunctionRule(ctx context.Context, projectID, service, function string) (*config.Rule, error) {
	if m.funcRules == nil {
	...
	if defaultServiceStub, p := m.funcRules[resourceID]; p && defaultServiceStub.Endpoints != nil {
