...
func TestModule_matchEncrypt(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			m:            &Module{aesKey: base64DecodeString("Olw6AhA/GzSxfhwKLxO7JJsUL6VUwwGEFTgxzoZPy9g=")},
			args:         args{rule: &config.Rule{Rule: "encrypt", Fields: []string{"args.username"}, Clause: &config.Rule{Rule: "allow"}}, args: map[string]interface{}{"args": map[string]interface{}{"username": "username1"}}},
			want:         &model.PostProcess{},
			shouldChange: true,
		},
		{
...
func TestModule_matchDecrypt(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name:         "valid args",
			m:            &Module{aesKey: base64DecodeString("Olw6AhA/GzSxfhwKLxO7JJsUL6VUwwGEFTgxzoZPy9g=")},
			args:         args{rule: &config.Rule{Rule: "decrypt", Fields: []string{"args.username"}}, args: map[string]interface{}{"args": map[string]interface{}{"username": "BXioRN4GyvZs"}}},
			want:         &model.PostProcess{},
			shouldChange: true,
		},
		{
		...
		},
		{
			name:         "valid args with rule allow",
			m:            &Module{aesKey: base64DecodeString("Olw6AhA/GzSxfhwKLxO7JJsUL6VUwwGEFTgxzoZPy9g=")},
			args:         args{rule: &config.Rule{Rule: "decrypt", Fields: []string{"args.username"}, Clause: &config.Rule{Rule: "allow"}}, args: map[string]interface{}{"args": map[string]interface{}{"username": "BXioRN4GyvZs"}}},
			want:         &model.PostProcess{},
			shouldChange: true,
		},
		{
...
func Test_matchHash(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "valid args",
			args:         args{rule: &config.Rule{Rule: "hash", Fields: []string{"args.password"}}, args: map[string]interface{}{"args": map[string]interface{}{"password": "password"}}},
			want:         &model.PostProcess{},
			shouldChange: true,
		},
		{
		...
		{
			name:         "valid args with rule allow",
			args:         args{rule: &config.Rule{Rule: "hash", Fields: []string{"args.password"}, Clause: &config.Rule{Rule: "allow"}}, args: map[string]interface{}{"args": map[string]interface{}{"password": "password"}}},
			want:         &model.PostProcess{},
			shouldChange: true,
		},
		{
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				...
				rule: &config.Rule{Rule: "webhook", URL: "http://localhost/validate", Claims: map[string]interface{}{"role": "admin", "serviceAddr": "http://localhost:9000/"}, Template: `{"service":"{{.args.auth.serviceAddr}}"}`},
				args: map[string]interface{}{"args": map[string]interface{}{"auth": map[string]interface{}{}, "token": "loremparis"}}},
			want:    nil,
			wantErr: false,
		},
