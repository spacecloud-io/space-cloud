...
func TestMatch_Rule(t *testing.T) {
	...
	err := auth.SetConfig("default", map[int]string{}, "Olw6AhA/GzSxfhwKLxO7JJsUL6VUwwGEFTgxzoZPy9g=", rule, &config.FileStore{}, &config.ServicesModule{}, &config.Eventing{})
	if err != nil {
...
func TestMatchForce_Rule(t *testing.T) {
	...
	_ = auth.SetConfig("default", map[int]string{}, "Olw6AhA/GzSxfhwKLxO7JJsUL6VUwwGEFTgxzoZPy9g=", config.Crud{}, &config.FileStore{}, &config.ServicesModule{}, &config.Eventing{})
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
