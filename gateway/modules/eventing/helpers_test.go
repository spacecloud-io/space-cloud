...
func TestModule_validate(t *testing.T) {
	...
	err := authModule.SetConfig("project", map[string]string{"0": "mySecretkey"}, "", config.Crud{}, &config.FileStore{}, &config.ServicesModule{}, &config.Eventing{SecurityRules: map[string]*config.Rule{"event": &config.Rule{Rule: "authenticated"}}})
	if err != nil {
