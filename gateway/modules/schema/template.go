...
func generateSDL(schemaCol model.Collection) (string, error) {
	...
	{{$fieldName}}: {{if eq $fieldValue.Kind "Object"}}{{$fieldValue.JointTable.Table}}{{else}}{{$fieldValue.Kind}}{{end}}{{if $fieldValue.IsFieldTypeRequired}}!{{end}} {{if $fieldValue.IsPrimary}}@primary{{end}}    {{if $fieldValue.PrimaryKeyInfo}} {{if and $fieldValue.PrimaryKeyInfo.IsAutoIncrement $fieldValue.PrimaryKeyInfo.Order}}  (autoIncrement: {{$fieldValue.PrimaryKeyInfo.IsAutoIncrement}},order: {{$fieldValue.PrimaryKeyInfo.Order}} ) {{else if $fieldValue.PrimaryKeyInfo.IsAutoIncrement}}  (autoIncrement: {{$fieldValue.PrimaryKeyInfo.IsAutoIncrement}}) {{else if $fieldValue.PrimaryKeyInfo.Order}}  (order: {{$fieldValue.PrimaryKeyInfo.Order}}) {{end}}  {{end}}  {{if eq $fieldValue.Kind "ID"}}@size(value: {{$fieldValue.TypeIDSize}}){{end}} {{if $fieldValue.IsCreatedAt}}@createdAt{{end}} {{if $fieldValue.IsUpdatedAt}}@updatedAt{{end}} {{if $fieldValue.IsUnique}}@unique(group: "{{$fieldValue.IndexInfo.Group}}", order: {{$fieldValue.IndexInfo.Order}})  {{else}} {{if $fieldValue.IsIndex}}@index(group: "{{$fieldValue.IndexInfo.Group}}", sort: "{{$fieldValue.IndexInfo.Sort}}", order: {{$fieldValue.IndexInfo.Order}}){{end}}{{end}} {{if $fieldValue.IsDefault}}@default(value: {{$fieldValue.Default}}){{end}} {{if $fieldValue.IsLinked}}@link(table: {{$fieldValue.LinkedTable.Table}}, from: {{$fieldValue.LinkedTable.From}}, to: {{$fieldValue.LinkedTable.To}}, field: {{$fieldValue.LinkedTable.Field}}){{end}} {{if $fieldValue.IsForeign}}@foreign(table: {{$fieldValue.JointTable.Table}}, field: {{$fieldValue.JointTable.To}}{{if eq $fieldValue.JointTable.OnDelete "CASCADE"}} ,onDelete: cascade{{end}}){{end}}{{end}}{{end}}
}`
