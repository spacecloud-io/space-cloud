...
func (m *Modules) SetProjectConfig(c *config.Project) error {
	...
	helpers.Logger.LogDebug(helpers.GetRequestID(nil), "Setting config of db module", nil)
	if err := m.db.SetConfig(p.ID, p.Modules.Crud); err != nil {
		_ = helpers.Logger.LogError(helpers.GetRequestID(nil), "Unable to set db module config", err, nil)
	}

	helpers.Logger.LogDebug(helpers.GetRequestID(nil), "Setting config of schema module", nil)
	if err := m.schema.SetConfig(p.Modules.Crud, p.ID); err != nil {
		_ = helpers.Logger.LogError(helpers.GetRequestID(nil), "Unable to set schema module config", err, nil)
	}

	helpers.Logger.LogDebug(helpers.GetRequestID(nil), "Setting config of auth module", nil)
	if err := m.auth.SetConfig(p.ID, p.SecretSource, p.Secrets, p.AESKey, p.Modules.Crud, p.Modules.FileStore, p.Modules.Services, &p.Modules.Eventing); err != nil {
		_ = helpers.Logger.LogError(helpers.GetRequestID(nil), "Unable to set auth module config", err, nil)
	}

	helpers.Logger.LogDebug(helpers.GetRequestID(nil), "Setting config of functions module", nil)
	if err := m.functions.SetConfig(p.ID, p.Modules.Services); err != nil {
		_ = helpers.Logger.LogError(helpers.GetRequestID(nil), "Unable to set remote services module config", err, nil)
	}

	helpers.Logger.LogDebug(helpers.GetRequestID(nil), "Setting config of user management module", nil)
	m.user.SetConfig(p.Modules.Auth)

	helpers.Logger.LogDebug(helpers.GetRequestID(nil), "Setting config of file storage module", nil)
	if err := m.file.SetConfig(p.ID, p.Modules.FileStore); err != nil {
		_ = helpers.Logger.LogError(helpers.GetRequestID(nil), "Unable to set filestore module config", err, nil)
	}

	helpers.Logger.LogDebug(helpers.GetRequestID(nil), "Setting config of eventing module", nil)
	if err := m.eventing.SetConfig(p.ID, &p.Modules.Eventing); err != nil {
		_ = helpers.Logger.LogError(helpers.GetRequestID(nil), "Unable to set eventing module config", err, nil)
	}

	helpers.Logger.LogDebug(helpers.GetRequestID(nil), "Setting config of realtime module", nil)
	if err := m.realtime.SetConfig(p.ID, p.Modules.Crud); err != nil {
		_ = helpers.Logger.LogError(helpers.GetRequestID(nil), "Unable to set realtime module config", err, nil)
	}

	helpers.Logger.LogDebug(helpers.GetRequestID(nil), "Setting config of graphql module", nil)
	m.graphql.SetConfig(p.ID)

	helpers.Logger.LogDebug(helpers.GetRequestID(nil), "Setting config of lets encrypt module", nil)
	if err := m.GlobalMods.LetsEncrypt().SetProjectDomains(p.ID, p.Modules.LetsEncrypt); err != nil {
		_ = helpers.Logger.LogError(helpers.GetRequestID(nil), "Unable to set letsencypt module config", err, nil)
	}

	helpers.Logger.LogDebug(helpers.GetRequestID(nil), "Setting config of ingress routing module", nil)
	if err := m.GlobalMods.Routing().SetProjectRoutes(p.ID, p.Modules.Routes); err != nil {
		_ = helpers.Logger.LogError(helpers.GetRequestID(nil), "Unable to set routing module config", err, nil)
	}
...
func (m *Modules) SetCrudConfig(projectID string, crudConfig config.Crud) error {
	...
	if err := m.db.SetConfig(projectID, crudConfig); err != nil {
		return helpers.Logger.LogError(helpers.GetRequestID(nil), "Unable to set db module config", err, nil)
	}
	helpers.Logger.LogDebug(helpers.GetRequestID(nil), "Setting config of auth module", nil)
	...
	if err := m.schema.SetConfig(crudConfig, projectID); err != nil {
		return helpers.Logger.LogError(helpers.GetRequestID(nil), "Unable to set schema module config", err, nil)
	}
	if err := m.realtime.SetConfig(projectID, crudConfig); err != nil {
		return helpers.Logger.LogError(helpers.GetRequestID(nil), "Unable to set realtime module config", err, nil)
	}
	helpers.Logger.LogDebug(helpers.GetRequestID(nil), "Setting config of file storage module", nil)
	return nil
}
...
func (m *Modules) SetServicesConfig(projectID string, services *config.ServicesModule) error {
	...
	helpers.Logger.LogDebug(helpers.GetRequestID(nil), "Setting config of remote services module", nil)
...
func (m *Modules) SetFileStoreConfig(projectID string, fileStore *config.FileStore) error {
	...
	if err := m.file.SetConfig(projectID, fileStore); err != nil {
		return helpers.Logger.LogError(helpers.GetRequestID(nil), "Unable to set filestore module config", err, nil)
	}
	return nil
}
...
func (m *Modules) SetEventingConfig(projectID string, eventingConfig *config.Eventing) error {
	...
	if err := m.eventing.SetConfig(projectID, eventingConfig); err != nil {
		return helpers.Logger.LogError(helpers.GetRequestID(nil), "Unable to set eventing module config", err, nil)
	}
