...
func (s *Server) routes(profiler bool, staticPath string, restrictedHosts []string) http.Handler {
	...
	router.Methods(http.MethodGet).Path("/v1/config/global-config").HandlerFunc(handlers.HandleGetGlobalConfig(s.adminMan, s.syncMan))
	router.Methods(http.MethodPost).Path("/v1/config/global-config").HandlerFunc(handlers.HandleSetGlobalConfig(s.adminMan, s.syncMan))
