...
func (s *Server) routes(profiler bool, staticPath string, restrictedHosts []string) http.Handler {
	...
	router.Methods(http.MethodGet).Path("/v1/config/global-config").HandlerFunc(handlers.HandleGetClusterConfig(s.adminMan, s.syncMan))
	router.Methods(http.MethodPost).Path("/v1/config/global-config").HandlerFunc(handlers.HandleSetClusterConfig(s.adminMan, s.syncMan))
