...
func NewDockerDriver(auth *auth.Module, clusterName, artifactAddr string) (*Docker, error) {
	...
	if err != nil {
		return nil, helpers.Logger.LogError(helpers.GetInternalRequestID(), "error creating docker module instance in docker in docker unable to initialize docker client", err, nil)
	}
	...
	if hostFilePath == "" {
		helpers.Logger.LogFatal(helpers.GetInternalRequestID(), "Failed to create docker driver: HOSTS_FILE_PATH environment variable not provided", nil)
	}
