...
func (s *Server) routes() {
	...
	// service routes
	s.router.Methods(http.MethodPut).Path("/v1/runner/services").HandlerFunc(s.handleApplyService())
	...
	// secret routes
	s.router.Methods(http.MethodPut).Path("/v1/runner/{project}/secrets").HandlerFunc(s.handleApplySecret())
	s.router.Methods(http.MethodGet).Path("/v1/runner/{project}/secrets").HandlerFunc(s.handleListSecrets())
	s.router.Methods(http.MethodPut).Path("/v1/runner/{project}/secrets/{name}/root-path").HandlerFunc(s.handleSetFileSecretRootPath())
	s.router.Methods(http.MethodDelete).Path("/v1/runner/{project}/secrets/{name}").HandlerFunc(s.handleDeleteSecret())
	s.router.Methods(http.MethodPut).Path("/v1/runner/{project}/secrets/{name}/{key}").HandlerFunc(s.handleSetSecretKey())
