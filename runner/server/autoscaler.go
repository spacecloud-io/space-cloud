...
func (s *Server) aggregate() {
	...
	if err := stream.Orchestrate(context.Background()); err != nil {
		_ = helpers.Logger.LogError(helpers.GetInternalRequestID(), "Could start stream from badger:", err, nil)
		return
	}
	...
	a60.iterate(func(project, service, version string, value int32) {
		...
		go func() {
			...
			if err := s.driver.AdjustScale(ctx, &model.Service{ProjectID: project, ID: service, Version: version}, value); err != nil {
				_ = helpers.Logger.LogError(helpers.GetInternalRequestID(), fmt.Sprintf("Could not adjust scale of service (%s:%s)", project, service), err, nil)
			}
		}()
		...
		case <-ticker.C:
			if len(messages) > 0 {
				if err := s.flushMetrics(messages); err != nil {
					_ = helpers.Logger.LogError(helpers.GetInternalRequestID(), "Could not flush metrics to disk:", err, nil)
				}
