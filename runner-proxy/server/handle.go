...
func (s *Server) handleProxy() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !exist {
			...
			var vPtr interface{}
			url := fmt.Sprintf("http://runner.space-cloud.svc.cluster.local:4050/v1/runner/%s/scale-up/%s/%s", project, service, ogVersion)
			...
			url = fmt.Sprintf("http://runner.space-cloud.svc.cluster.local:4050/v1/runner/%s/wait/%s/%s", project, service, ogVersion)
