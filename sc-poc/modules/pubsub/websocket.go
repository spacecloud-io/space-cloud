...
func (a *App) getSubscriberAPI(channelPath, channelName string) *apis.API {
	return &apis.API{
		...
		Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			subscribeStatus := make(chan bool)
			...
			for startSubscribe := range subscribeStatus {
				if startSubscribe {
					...
					for {
						select {
						case status := <-subscribeStatus:
							if !status {
								// TODO: Stop subscribing and break out of loop
								a.logger.Debug("stop subscribing")
							}
						case msg := <-msgChan:
